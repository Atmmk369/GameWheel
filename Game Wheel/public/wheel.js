...
function spinWheel() {
  ...
  const targetPosition = -(extraRotations * totalHeight + randomIndex * itemHeight) + (itemHeight / 2);
  
  // Start the animation after a short delay to allow visual scrolling
  setTimeout(() => {
    // Stop scrolling animation
    itemsStrip.classList.remove('scrolling');
    
    // Start the smooth transition to final position
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        const resetPosition = -(randomIndex * itemHeight) + (itemHeight / 2);
        itemsStrip.style.transform = `translateY(${resetPosition}px)`;
      }, 100);
    }, spinDuration);
  }, 1000); // Let scrolling animation run for 1 second
}
